...
export function ClientPortal({ addresses = [], appName, contactEmail = "", contactPhone = "", markNotificationRead, notifications = [], onLogout, orders, removeAddress, saveAddress, saveProfile, saveShop, setDefaultAddress, shops = [], submitOrder, themeProps, user }) {
  ...
  return (
    <div className="mobile-app">
      ...
      <main className="mobile-content">
        {page === "home" && (
          <ClientHome activeOrder={activeOrder} contactEmail={contactEmail} contactPhone={contactPhone} onCreate={() => setPage("new")} onTrack={openTracking} orders={orders} user={user} />
        )}
...
function ClientHome({ activeOrder, contactEmail, contactPhone, onCreate, onTrack, orders, user }) {
...
function NewRequest({ addresses = [], onCancel, onSubmit, shops = [], user }) {
  ...
  const [form, setForm] = useState({
    shopId: defaultShop?._apiId || "",
    pickupContact: defaultShop?.contactName || defaultShop?.name || "",
    pickupPhone: defaultShop?.phone || "",
    pickup: defaultShop?.address || "",
    receiver: defaultAddress?.recipientName || "",
    receiverPhone: defaultAddress?.phone || "",
    destination: defaultAddress?.address || "",
    product: "",
    category: "Package",
    quantity: "1",
    fragile: false,
  ...
  return (
    <section className="request-flow">
      ...
      <div className="form-content">
        ...
        {step === 4 && (
          <>
            <FormIntro icon="wallet" title="Delivery fee payment" text="Delivery fees are collected in cash when the rider completes the delivery. You can optionally note a preferred amount." />
            <label className="field-label">Fee payment method</label>
            <div className="payment-options">
              {["Cash on delivery", "Cash"].map((method) => (
                <button
        ...
        {step === 5 && (
          <>
            ...
            <ReviewSection label="Payment" title={form.paymentMethod} lines={[form.fee ? `Optional estimate: ${Number(form.fee).toLocaleString()} MMK` : "Final fee set by rider after delivery"]} />
          </>
        )}
      </div>
